...
const reduxSetState = require('set-state-redux')
...
it('works with other redux decorators', () => {
  const createStore = pipe(
    () => redux.createStore,
    reduxSetState.decorateCreateStore,
    decorateCreateStore
  )
  const combineReducers = decorateCombineReducers(redux.combineReducers)
  ...
  state.cars = state.cars.concat(['porsche'])
  store.setState(state)
  expect(store.getState()).toEqual({cars: [ 'ferrari', 'porsche' ], books: ['moby dick']})
})
